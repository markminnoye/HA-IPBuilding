...
from .const import DOMAIN, TYPE_TIME, TYPE_REGIME, TYPE_RELAY, TYPE_DIMMER, FAST_POLL_TYPES, SLOW_POLL_TYPES
...
async def async_setup_entry(
    ...
    # 1. Fetch Fast Poll Sensors (Time, etc defined in FAST_POLL_TYPES)
    # Actually, we should iterate over which sensor types are relevant here.
    # Currently handled: Time, Regime (moved to Slow), Power (from Relay/Dimmer)
    
    # Fetch Time sensors (Fast)
    try:
    ...
    # Fetch Regime sensors (Slow - per user request)
    try:
    ...
    # Fetch Power sensors (from Relays and Dimmers - Fast)
    # Note: Power sensors are derived from Relays/Dimmers. They share the same hardware device.
    # We use the FAST coordinator because we want instant power updates when a light turns on.
    for type_id in [TYPE_RELAY, TYPE_DIMMER]:
        try:
            devices = await api.get_devices(type_id)
            for device in devices:
                if "Watt" in device:
                    entities.append(IPBuildingPowerSensor(coordinator_fast, api, device))
        except Exception as e:
